<p align="center">
  <img src="docs/assets/banner.png" alt="AnythingMCP — 188 个连接器，其中 26 个无需 API 密钥。将 REST、SOAP/WSDL、GraphQL、SQL 和 MCP 系统转化为 Claude、ChatGPT、Copilot 和 Gemini 的工具。" width="100%" />
</p>
...
<p align="center">
  ...
  188 个现成适配器，无需编写代码即可连接任意 REST、SOAP、GraphQL 或 SQL 系统，全部运行在你自己的基础设施上，并能学习各个系统之间的关系。
</p>
...
- **适配器（adapter）**：本仓库随附的 188 个 JSON 定义之一，例如 DATEV、weclapp、DHL、Deutsche Bahn、Shopware、Personio、Handelsregister 等。其中 26 个完全不需要 API 密钥，其余适配器会在导入时要求你提供相应凭据。
...
188 个适配器，提供 1,800 多个工具。**其中 26 个不需要 API 密钥**，其余适配器会在导入时要求提供凭据，导入后工具即可立即使用。每个适配器都在 [anythingmcp.com/guides](https://anythingmcp.com/guides) 上提供七种语言的设置指南。
