...
CREATE TABLE orders (
  ...
  status VARCHAR(12) NOT NULL CHECK (status IN ('OPEN','SHIPPED','INVOICED','CANCELLED'))
);
...
CREATE USER 'amcp_reader'@'%' IDENTIFIED BY 'amcp_reader';
GRANT SELECT ON shop.* TO 'amcp_reader'@'%';
FLUSH PRIVILEGES;
